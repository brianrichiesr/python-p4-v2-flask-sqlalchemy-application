...
@app.route('/species/<string:pet_species>')
def pet_by_species(pet_species):
    pets = Pet.query.filter(Pet.species == pet_species.title()).all()
    print(f"{pet_species}")
    size = len(pets)  # all() returns a list so we can get length
    response_body = f'<h2>There are {size} {pet_species}s</h2>'
    for pet in pets:
        response_body += f'<p>{pet.name} {pet.species}</p>'
